...
**Permission hooks (`src/hooks/usePermissions.ts`).** The auth store holds the current user (with `permissions` for admins, delivered by `/users/me`). `usePermissions()` returns `{ isSuperadmin, permissions, has(permission) }`, alongside ergonomic one-liners — `useCanReadUsers`, `useCanWriteUsers`, `useCanChangeUserRole`, `useCanDeleteUsers`, `useCanDeleteFiles`, `useCanWriteSupport`, `useCanUpdateSupport`, … — each returning a boolean (superadmins always `true`).
...
3. **Action** — buttons (delete, suspend, reset password, change role, reassign ticket, …) appear only with the matching permission. Where one is hidden, a small hint makes clear it's intentional and **no request is sent**.
...
**Admin management (`/admin/admins`, superadmin-only).** List admins, promote a user by email with an initial grant set, edit a permission matrix (grouped by resource, with section search), and demote — all through a modal-driven UI.
...
- [ ] Visit `/admin/admins` to promote a user to admin and assign permissions via the matrix
